...
def parse_amount_from_long_enough_bytes(subscript: bytes, amount_size):
    ...
    if len(subscript) < amount_size:
        raise ValueError(
            "Too short bytes for amount; less than the provided size ({} < {})".format(len(subscript), amount_size))
...
def parse_ntp1v3_metadata_from_long_enough_string(subscript: bytes) -> bytes:
    ...
    if len(subscript) < 4:
        raise ValueError("The data remaining cannot fit metadata start flag, which is 4 bytes, starting from {}".format(
            subscript.hex()))
...
def parse_ntp1v1_burn_data(op_code, subscript: bytes) -> dict:
    result = {}
    ptr = 0
    ...
    if len(subscript[ptr:]) < metadata_size:
        raise ValueError("Invalid metadata size; smaller than expected (expected {}, found {})".format(metadata_size,
                                                                                                       len(subscript[
                                                                                                           5:])))
...
def run_burn_ntp1v1_parsing_tests():
    transfer_script = '4e5401251f2013'
...
def run_transfer_ntp1v1_parsing_tests():
    ...
    assert_equal(transfer_script_parse_result['transfer_data']['transfer_instructions'][0].raw_amount,
                 bytes.fromhex('69892A92'))
...
def run_issuance_ntp1v1_parsing_tests():
    ...
    assert_equal(nibbl_issuance_parse_result['issuance_data']['metadata'], bytes.fromhex(
        "AB10C04E20E0AEC73D58C8FBF2A9C26A6DC3ED666C7B80FEF215620C817703B1E5D8B1870211CE7CDF50718B4789245FB80F5899"))
    ...
    assert_equal(nibbl_issuance_parse_result['issuance_data']['transfer_instructions'][0].raw_amount,
                 bytes.fromhex('2019'))
    ...
    assert_equal(nibbl_issuance_parse_result['issuance_data']['issuance_flags'].aggregation_policy,
...
def run_issuance_ntp1v3_parsing_tests():
    ...
    assert_equal(issuance_parse_result['issuance_data']['metadata'], bytes.fromhex(
        ""))
    ...
    assert_equal(issuance_parse_result['issuance_data']['issuance_flags'].aggregation_policy,
                 AggregationPolicy.Aggregatable)
