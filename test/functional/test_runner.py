...
#    'wallet_hd.py',
   'wallet_backup.py',
    ...
    'feature_block_viu_cache_ca_shift_down.py',
    'feature_block_viu_cache_ca_shift_up.py',
    'feature_block_viu_cache_ca_shift_back_and_forth.py',
...
def main():
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and nebliod must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if nebliod is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "nebliod"]) is not None:
            print("%sWARNING!%s There is already a nebliod process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/nebliod' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/neblio-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie nebliods, we can apply a
...
class RPCCoverage():
    ...
    commands per `neblio-cli help` (`rpc_interface.txt`).
