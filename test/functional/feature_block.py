...
class FullBlockTest(ComparisonTestFramework):
    ...
    def get_tests(self):
        ...
        #                      \-> b3 (1) -> b4 (2)
        tip(6)
        block(9, spend=out[4], additional_coinbase_value=10 * COIN)
        ...
        block(11, spend=out[4], additional_coinbase_value=10 * COIN)
        yield rejected(RejectResult(16, b'bad-cb-amount'))
        ...
        #                      \-> b3 (1) -> b4 (2)
        tip(15)
        block(20, spend=PreviousSpendableOutput(self.blocks[15].vtx[0], 0))
        yield rejected(RejectResult(16, b'bad-txns-premature-spend-of-coinbase/coinstake'))
        ...
        block(22, spend=PreviousSpendableOutput(self.blocks[15].vtx[0], 0))
        yield rejected()

        # self.write_graph_to_file(self.create_blockchain_graph(), "graph.dot")
        ...
        tx = CTransaction()
        script_length = MAX_BLOCK_BASE_SIZE - len(b23.serialize()) - 78
        ...
        # Make the next block a little bigger and check that it fails
        tip(15)
        b24 = block(24, spend=out[6])
        script_length = MAX_BLOCK_BASE_SIZE - len(b24.serialize()) - 60
        ...
        tip(23)
        b30 = block(30)
        # we add 0x00's to scriptSig until its size is 100
        b30.vtx[0].vin[0].scriptSig = b30.vtx[0].vin[0].scriptSig + b'\x00' * (100 - len(b30.vtx[0].vin[0].scriptSig))
        ...
        spend = out[11]
        # the amount is 1 + fee because in the next test we'll spend these outputs and we need fees for that
        tx = create_tx(spend.tx, spend.n, 1 + fee, p2sh_script)
        tx.vout.append(CTxOut(spend.tx.vout[spend.n].nValue - 1 - 2*fee, CScript([OP_TRUE])))
        self.sign_tx(tx, spend.tx, spend.n)
        tx.rehash()
        ...
        while total_size < MAX_BLOCK_BASE_SIZE:
            # two transactions will come out of these outputs, this is why we have a fee here + 2*fee
            tx_new = create_tx(tx_last, 1, 1 + 2*fee, p2sh_script)
            tx_new.vout.append(CTxOut(tx_last.vout[1].nValue - 1 - 3*fee, CScript([OP_TRUE])))
            tx_new.rehash()
            total_size += len(tx_new.serialize())
            if total_size + 5 >= MAX_BLOCK_BASE_SIZE:  # +5 to account for vchBlockSig which can vary in size
                break
        ...
        for i in range(1, numTxes+1):
            tx = CTransaction()
            tx.vout.append(CTxOut(1 + fee, CScript([OP_TRUE])))
        ...
        tip(55)
        b57 = block(57)
        tx = create_and_sign_tx(out[16].tx, out[16].n, 3*fee + 1)
        tx1 = create_tx(tx, 0, 2*fee + 1)
        ...
        tip(55)
        b57p2 = block("57p2")
        tx = create_and_sign_tx(out[16].tx, out[16].n, 5*fee+1)
        tx1 = create_tx(tx, 0, 4*fee+1)
        tx2 = create_tx(tx1, 0, 3*fee+1)
        tx3 = create_tx(tx2, 0, 2*fee+1)
        tx4 = create_tx(tx3, 0, fee+1)
