...
TEST(ntp1_tests, parsig_ntp1_from_ctransaction_issuance)
{
    ...
    for (auto&& input : inputs) {
        input.second.readNTP1DataFromTx_minimal(dbMock->GetBestChainHeight(), input.first);
    }

    NTP1Transaction ntp1tx;
    EXPECT_NO_THROW(ntp1tx.readNTP1DataFromTx(dbMock->GetBestChainHeight(), tx, inputs));
...
TEST(ntp1_tests, parsig_ntp1_from_ctransaction_transfer_1)
{
    ...
    NTP1Transaction ntp1tx;
    ASSERT_NO_THROW(ntp1tx.readNTP1DataFromTx(dbMock->GetBestChainHeight(), tx, inputs));
...
void TestNTP1TxParsing_onlyRead(const CTransaction& tx, NetworkType netType)
{
    ...
    NTP1Transaction ntp1tx;
    ntp1tx.readNTP1DataFromTx(dbMock->GetBestChainHeight(), tx, inputs);
}
...
void TestNTP1TxParsing(const CTransaction& tx, NetworkType netType)
{
    ...
    EXPECT_TRUE(tx.CheckTransaction(dbMock->GetBestChainHeight()).isOk()) << "Failed tx: " << txid;
                                    ...
                                    const flat_map<std::string, std::string>& ntp1txs_map)
{
    ...
    boost::shared_ptr<mTxDB> dbMock = boost::make_shared<mTxDB>();
    EXPECT_CALL(*dbMock, GetBestChainHeight()).WillRepeatedly(testing::Return(10000000));
...
TEST(ntp1_tests, amend_tx_1)
{
    ...
    NTP1Transaction::AmendStdTxWithNTP1(dbMock->GetBestChainHeight(), tx, inputs, 1);
...
TEST(ntp1_tests, amend_tx_with_op_return)
{
    ...
    auto inputs = GetNTP1InputsOnline(tx, NetworkType::Testnet);
    EXPECT_ANY_THROW(NTP1Transaction::AmendStdTxWithNTP1(dbMock->GetBestChainHeight(), tx, inputs, 1));
}
