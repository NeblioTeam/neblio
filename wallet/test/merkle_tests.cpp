...
{
    uint256 hash = leaf;
    for (std::vector<uint256>::const_iterator it = vMerkleBranch.begin(); it != vMerkleBranch.end();
         ++it) {
        if (nIndex & 1) {
...
static void MerkleComputation(const std::vector<uint256>& leaves, uint256* proot, bool* pmutated,
                              uint32_t branchpos, std::vector<uint256>* pbranch)
{
    if (pbranch)
        pbranch->clear();
    if (leaves.size() == 0) {
    ...
    while (count < leaves.size()) {
        ...
        for (level = 0; !(count & (((uint32_t)1) << level)); level++) {
            ...
            h = Hash(inner[level].begin(), inner[level].end(), h.begin(), h.end());
        }
...
TEST(merkle_tests, merkle_test)
{
    for (int i = 0; i < 32; i++) {
        // Try 32 block sizes: all sizes from 0 to 16 inclusive, and then 15 random sizes.
        int ntx = (i <= 16) ? i : 17 + (GetRandInt(4000));
        ...
        for (int mutate = 0; mutate <= 3; mutate++) {
            ...
            EXPECT_TRUE(oldRoot == newRoot);
            EXPECT_TRUE(newRoot == unmutatedRoot);
            EXPECT_TRUE((newRoot == uint256()) == (ntx == 0));
            EXPECT_TRUE(oldMutated == newMutated);
            EXPECT_TRUE(newMutated == !!mutate);
            ...
            if (mutate == 0) {
                for (int loop = 0; loop < std::min(ntx, 16); loop++) {
                    ...
                    EXPECT_TRUE(oldBranch == newBranch);
                    EXPECT_TRUE(ComputeMerkleRootFromBranch(block.vtx[mtx].GetHash(), newBranch, mtx) ==
                                oldRoot);
                }
            }
        }
...
TEST(merkle_tests, merkle_test_empty_block)
{
    bool    mutated = false;
    CBlock  block;
    uint256 root = BlockMerkleRoot(block, &mutated);

    EXPECT_EQ(root.IsNull(), true);
    EXPECT_EQ(mutated, false);
}

TEST(merkle_tests, merkle_test_oneTx_block)
{
    bool   mutated = false;
    CBlock block;
    ...
    CTransaction mtx;
    mtx.nLockTime = 0;
    block.vtx[0]  = std::move(mtx);
    uint256 root  = BlockMerkleRoot(block, &mutated);
    EXPECT_EQ(root, block.vtx[0].GetHash());
    EXPECT_EQ(mutated, false);
}

TEST(merkle_tests, merkle_test_OddTxWithRepeatedLastTx_block)
{
    bool   mutated;
    ...
    EXPECT_EQ(rootofBlock, rootofBlockWithRepeatedLastTx);
    EXPECT_EQ(mutated, true);
}

TEST(merkle_tests, merkle_test_LeftSubtreeRightSubtree)
{
