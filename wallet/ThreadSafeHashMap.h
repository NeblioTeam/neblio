...
public:
    ...
    void                                           clear();
    [[nodiscard]] std::unordered_map<K, V, Hasher> getInternalMap() const;
...
template <typename K, typename V, typename Hasher>
boost::optional<V> ThreadSafeHashMap<K, V, Hasher>::get(const K& key) const
{
