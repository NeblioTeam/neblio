                                                    ...
                                                    const CCoinControl*              coinControl)
{
...

    {
        ...
        try {
            ...
            ntp1tx.readNTP1DataFromTx(txdb.GetBestChainHeight().value_or(0), wtx, inputsTxs);
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput>>& mapCoins) const
{
    ...
    for (const COutput& out : vCoins) {
        ...
        if (!ExtractDestination(txdb.GetBestChainHeight().value_or(0),
                                cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
