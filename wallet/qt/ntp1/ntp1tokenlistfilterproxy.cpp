...
bool NTP1TokenListFilterProxy::filterAcceptsRow(int source_row, const QModelIndex &source_parent) const
{
    ...
    if(sourceModel()->data(columnsIndex, NTP1TokenListModel::AmountRole).toString().contains(filterLineEdit->text(), Qt::CaseInsensitive) ||
       sourceModel()->data(columnsIndex, NTP1TokenListModel::TokenDescriptionRole).toString().contains(filterLineEdit->text(), Qt::CaseInsensitive) ||
       sourceModel()->data(columnsIndex, NTP1TokenListModel::TokenNameRole).toString().contains(filterLineEdit->text(), Qt::CaseInsensitive))
    {
        return true;
    } else {
        return false;
