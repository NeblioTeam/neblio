    ...
    : QDialog(parent), ui(new Ui::SignVerifyMessageDialog), model(0)
{
    ...
    ui->addressIn_SM->setPlaceholderText(
        tr("Enter a neblio address (e.g. 8dpZqgY4r2RoEdqYk3QsAqFckyf9pRHN6i)"));
    ui->signatureOut_SM->setPlaceholderText(tr("Click \"Sign Message\" to generate signature"));

    ui->addressIn_VM->setPlaceholderText(
        tr("Enter a neblio address (e.g. 8dpZqgY4r2RoEdqYk3QsAqFckyf9pRHN6i)"));
    ui->signatureIn_VM->setPlaceholderText(tr("Enter neblio signature"));
#endif
...
void SignVerifyMessageDialog::setModel(WalletModel* modelIn) { this->model = modelIn; }
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    if (!key.SignCompact(Hash(ss.begin(), ss.end()), vchSig)) {
        ui->statusLabel_SM->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_SM->setText(QString("<nobr>") + tr("Message signing failed.") +
                                    QString("</nobr>"));
        return;
    }
...
void SignVerifyMessageDialog::on_verifyMessageButton_VM_clicked()
{
    ...
    if (!(CBitcoinAddress(key.GetPubKey().GetID()) == addr)) {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_VM->setText(QString("<nobr>") + tr("Message verification failed.") +
                                    QString("</nobr>"));
        return;
    }
