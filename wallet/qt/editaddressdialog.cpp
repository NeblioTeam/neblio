...
void AddLedgerRowWorker::addRow(Ui::EditAddressDialog* ui, AddressTableModel* model,
                                QSharedPointer<AddLedgerRowWorker> workerPtr)
{
    auto address =
        model->addRow(AddressTableModel::ReceiveLedger, ui->labelEdit->text(), ui->addressEdit->text(),
                      ui->ledgerAccountEdit->text(), ui->ledgerIndexEdit->text());
...
EditAddressDialog::EditAddressDialog(Mode modeIn, QWidget* parent)
    : QDialog(parent), ui(new Ui::EditAddressDialog), mapper(0), mode(modeIn), model(0)
{
    ui->setupUi(this);
    ...
    connect(this->ui->ledgerAccountEdit, SIGNAL(textChanged(QString)), this,
            SLOT(updateLedgerPathLabel()));
    connect(this->ui->ledgerIndexEdit, SIGNAL(textChanged(QString)), this,
            SLOT(updateLedgerPathLabel()));
}
    ...
    case NewReceivingAddress:
        if (isLedger) {
            ...
            QTimer::singleShot(0, worker.data(),
                               [this, worker]() { worker->addRow(ui, model, worker); });
            msgBox.exec();
        } else {
            address =
                model->addRow(AddressTableModel::Receive, ui->labelEdit->text(), ui->addressEdit->text(),
                              ui->ledgerAccountEdit->text(), ui->ledgerIndexEdit->text());
        }
        break;
    case NewSendingAddress:
    ...
    if (!saveCurrentRow()) {
        ...
        case AddressTableModel::INVALID_ADDRESS:
            QMessageBox::warning(this, windowTitle(),
                                 tr("The entered address \"%1\" is not a valid neblio address.")
        ...
        case AddressTableModel::INVALID_LEDGER_ACCOUNT:
            QMessageBox::warning(
                this, windowTitle(),
                tr("The entered Ledger account \"%1\" is not in the recommended range (0-%2).")
                    .arg(ui->ledgerAccountEdit->text())
                    .arg(ledger::MAX_RECOMMENDED_ACCOUNT),
                QMessageBox::Ok, QMessageBox::Ok);
            break;
        case AddressTableModel::INVALID_LEDGER_INDEX:
            QMessageBox::warning(
                this, windowTitle(),
                tr("The entered Ledger address index \"%1\" is not in the recommended range (0-%2).")
                    .arg(ui->ledgerIndexEdit->text())
                    .arg(ledger::MAX_RECOMMENDED_INDEX),
                QMessageBox::Ok, QMessageBox::Ok);
            break;
        case AddressTableModel::WALLET_UNLOCK_FAILURE:
            QMessageBox::critical(this, windowTitle(), tr("Could not unlock wallet."), QMessageBox::Ok,
                                  QMessageBox::Ok);
            break;
        case AddressTableModel::KEY_GENERATION_FAILURE:
            QMessageBox::critical(this, windowTitle(), tr("New key generation failed."), QMessageBox::Ok,
                                  QMessageBox::Ok);
            break;
        case AddressTableModel::LABEL_USED_BY_LEDGER:
            QMessageBox::critical(this, windowTitle(),
                                  tr("This label is already used by a Ledger address."), QMessageBox::Ok,
                                  QMessageBox::Ok);
            break;
        case AddressTableModel::LABEL_NOT_USABLE_FOR_LEDGER:
            QMessageBox::critical(this, windowTitle(), tr("Ledger addresses require a unique label."),
                                  QMessageBox::Ok, QMessageBox::Ok);
            break;
...
void EditAddressDialog::setAddress(QString addressIn) { this->address = addressIn; }
