...
public:
    ...
    static void        GetLargeFileFromHTTPS(
               const std::string& URL, long ConnectionTimeout, const boost::filesystem::path& targetPath,
               std::atomic<float>&                 progress,
               const std::unordered_set<CURLcode>& errorsToIgnore = std::unordered_set<CURLcode>());
