...
{
    return (lhs.tokenId == rhs.tokenId && lhs.issuanceTxId == rhs.issuanceTxId &&
            lhs.divisibility == rhs.divisibility && lhs.lockStatus == rhs.lockStatus &&
            lhs.aggregationPolicy == rhs.aggregationPolicy && lhs.numOfHolders == rhs.numOfHolders &&
            lhs.totalSupply == rhs.totalSupply && lhs.numOfTransfers == rhs.numOfTransfers &&
            lhs.numOfIssuance == rhs.numOfIssuance && lhs.numOfBurns == rhs.numOfBurns &&
            lhs.firstBlock == rhs.firstBlock && lhs.issueAddress == rhs.issueAddress &&
            lhs.tokenName == rhs.tokenName && lhs.tokenDescription == rhs.tokenDescription &&
            lhs.tokenIssuer == rhs.tokenIssuer && lhs.iconURL == rhs.iconURL &&
            lhs.iconImageType == rhs.iconImageType && lhs.sha2Issue == rhs.sha2Issue);
}
