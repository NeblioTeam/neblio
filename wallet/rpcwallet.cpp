...
Value getwatchonlybalance(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1)
        throw std::runtime_error(
            "getwatchonlybalance ( \"account\" )\n"
            "\nReturn the balance of the watch only account\n"
            "\nExamples:\n"
            "\nThe total amount in the wallet\n"
            "getwatchonlybalance \nAs a json rpc call\n"
            "getwatchonlybalance \"*\"");
    ...
    std::string strAccount = params[0].get_str();
    CAmount balance = GetAccountBalance(strAccount, /*nMinDepth*/ 1, ISMINE_WATCH_ONLY, CTxDB());
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin();
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            isminetype fMine = pwalletMain->IsMine(txout.scriptPubKey);
            if (!IsMineCheck(fMine, static_cast<isminetype>(filter)))
                continue;
            ...
            item.fWatchOnly = pwalletMain->IsMine(txout) & ISMINE_WATCH_ONLY;
        }
    }
    ...
    for (const auto& item : addrBook) {
        ...
        bool    fInvolvesWatchOnly = false;
        if (it != mapTally.end()) {
            nAmount = (*it).second.nAmount;
            nConf   = (*it).second.nConf;
            fInvolvesWatchOnly = (*it).second.fWatchOnly;
        }
        ...
        if (fByAccounts) {
            ...
            itemP.fWatchOnly = fInvolvesWatchOnly;
        } else {
            Object obj;
            ...
            obj.push_back(Pair("involvesWatchonly", fInvolvesWatchOnly));
    ...
    if (fByAccounts) {
        for (map<string, tallyitem>::iterator it = mapAccountTally.begin(); it != mapAccountTally.end();
            ...
            bool    fInvolvesWatchOnly = (*it).second.fWatchOnly;
            Object  obj;
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "[includeWatchonly=false] whether to include watch only payments"
                      ...
                      bool fLong, const isminefilter& filter, Array& ret)
{
    ...
    bool involvesWatchOnly = wtx.IsFromMe(ISMINE_WATCH_ONLY); // Means the inputs for the tx are from one of the WOA
    const uint256 bestBlockHash = txdb.GetBestBlockHash();
    ...
    if ((!wtx.IsCoinStake()) && (!listSent.empty() || nFee != 0) &&
        ...
        for (const COutputEntry& s : listSent) {
            Object entry;
            isminetype ismine = pwalletMain->IsMine(GetScriptForDestination(s.destination));
            ...
            if (involvesWatchOnly || (ismine & ISMINE_WATCH_ONLY))
    ...
    if (listReceived.size() > 0 && depthInMainChain >= nMinDepth) {
        ...
        for (const COutputEntry& r : listReceived) {
            string account;
            isminetype ismine = pwalletMain->IsMine(GetScriptForDestination(r.destination));
            if (const auto entry = pwalletMain->mapAddressBook.get(r.destination))
                account = entry->name;
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin();
        ...
        if (nDepth >= nMinDepth && wtx.GetBlocksToMaturity(txdb, bestBlockHash) == 0) {
            for (const COutputEntry& r : listReceived)
                if (const auto en = pwalletMain->mapAddressBook.get(r.destination))
                    mapAccountBalances[en->name] += r.amount;
                else
    ...
    Object ret;
    // TODO(sdmg15): Maybe change the way it returns here and include a key iswatchonly
...
Value validateaddress(const Array& params, bool fHelp)
{
    ...
    if (isValid) {
        ...
        mine = pwalletMain->IsMine(GetScriptForDestination(dest));

        ret.push_back(Pair("iswatchonly", IsMineCheck(mine, isminetype::ISMINE_WATCH_ONLY)));
...
Value validatepubkey(const Array& params, bool fHelp)
{
    ...
    if (isValid) {
        ...
        isminetype mine = pwalletMain->IsMine(GetScriptForDestination(dest));
        ret.push_back(Pair("iswatchonly", IsMineCheck(mine, isminetype::ISMINE_WATCH_ONLY)));
...
Value importaddress(const Array& params, bool fHelp) {
    ...
    if (fHelp || params.size() > 4)
        throw std::runtime_error(
                "importaddress [address] [label=\"\"] [rescan=true] [p2sh],"
    ...
    if (fRescan)
    {
        ...
        pwalletMain->ResendWalletTransactions(db);
    }
