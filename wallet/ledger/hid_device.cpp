...
int HID::recv(std::vector<uint8_t>& rdata)
{
    ...
    auto data_len =
        utils::bytes_to_int(std::vector<uint8_t>(data_chunk.begin() + 5, data_chunk.begin() + 7));
    ...
    auto sw =
        utils::bytes_to_int(std::vector<uint8_t>(data.begin() + data_len - 2, data.begin() + data_len));
...
bool HID::is_open() const { return opened_; }
