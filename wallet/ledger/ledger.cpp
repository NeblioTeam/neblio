...
namespace ledger {
Ledger::Ledger(Transport::TransportType transportType)
{
    this->transport = std::unique_ptr<Transport>(new Transport(transportType));
}
...
{
    return transport->exchange(APDU::CLA, APDU::INS_GET_TRUSTED_INPUT, firstRound ? 0x00 : 0x80, 0x00,
                               transactionData);
}
...
void Ledger::UntrustedHashTxInputStart(const Tx& tx, const std::vector<TrustedInput>& trustedInputs,
